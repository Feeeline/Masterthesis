...
Zur detaillierten Analyse der Prozessführung und zur Validierung der energetischen Ergebnisse sind in Tabelle [X] die Stoffstromdaten für beide Modellvarianten bei einem Referenzdruck von 8 bar (abs.) gegenübergestellt. Diese detaillierte Strombetrachtung verdeutlicht die thermodynamischen Ursachen für den in Abbildung 6.1 sichtbaren Effizienzvorteil der Doppelkolonne.
...
\paragraph{Kolonnenblock des Doppelkolonnen-model}\label{sec:modell_kolonne_double} Ziel des Doppelkolonnenprozesses, bestehend aus einer Hochdruckkolonne (HP-Kolonne, \textit{KOLHP}) und einer Niederdruckkolonne (LP-Kolonne, \textit{KOLLP}), ist die Erzeugung von gasförmigem Stickstoff unter Nutzung einer thermisch integrierten Kolonnenanordnung. Die grundlegenden thermodynamischen Annahmen entsprechen denen des Einkolonnenmodells und werden an dieser Stelle nicht erneut ausgeführt.\\
...
\paragraph{Integrierter Kältekreislauf des sauerstoffreichen Reststroms im Einkolonnenmodell} \label{sec:modell_loop} Im Einkolonnenmodell wird die erforderliche Kälteleistung zusätzlich über einen internen Reststrom-Kältekreis bereitgestellt. Der sauerstoffreiche Reststrom wird nach der Trennung entspannt und anschließend zur Vorkühlung des Luftfeeds im Hauptwärmeübertrager genutzt. 
...
Für die exergetische Bewertung wird ein einheitlicher Referenzzustand der Umgebung mit einer Temperatur von $T_0 = 288{,}15\,\mathrm{K}$ und einem Druck von $p_0 = 1{,}013\,\mathrm{bar}$ angenommen. Berücksichtigt werden ausschließlich physikalische und chemische Exergieanteile, während kinetische und potenzielle Beiträge aufgrund ihrer geringen Größenordnung vernachlässigt werden. Die Berechnung der chemischen Exergie erfolgt auf Basis des Referenzmodells nach Szargut unter Verwendung tabellierter Standardwerte der molaren chemischen Exergien der Reinstoffe. Die detaillierten Berechnungsgrundlagen werden im folgenden erläutert.
...
Ideale Verteiler (Splitter) verursachen definitionsgemäß keine Irreversibilitäten und werden nicht bilanziert.\\
Mischvorgänge können grundsätzlich zu Irreversibilitäten und damit zu Exergievernichtung führen, insbesondere beim Ausgleich von Temperatur-, Druck- oder Zusammensetzungsunterschieden. 
Die exergetische Bewertung basiert auf der Definition von Brennstoff- und Produktexergien für Apparate mit einer klaren thermodynamischen Funktion, d.\,h. einer gerichteten Umwandlung oder Übertragung von Exergie \cite{lazzaretto_speco_2006, tsatsaronis_definitions_2007}. Im vorliegenden Modell werden die betrachteten Mixer jedoch ausschließlich zur Zusammenführung von Stoffströmen verwendet, die auf gleichem Druckniveau, mit ähnlichem Temperaturniveau, sowie mit nahezu identischer Zusammensetzung (Produktqualität 5.0~Stickstoff) vorliegen. Eine eindeutige Definition von Brennstoff- und Produktexergie ist daher nicht sinnvoll möglich und die Exergievernichtung durch Mischung wird daher als vernachlässigbar klein eingestuft und nicht als eigenständiger Beitrag ausgewiesen.\\
...
In der ersten Variante wird ausschließlich der Kopfstrom $S23$ (Stickstoff-Destillat) als Produkt betrachtet. Der exergetische Produktnutzen ergibt sich als Differenz der chemischen und thermischen Exergie relativ zum Feedstrom $S11$:
\[
\dot{E}_P^{(1)}
=
\dot{m}_{23}
\left[
\left(e_{23}^{CH}-e_{11}^{CH}\right)
+
\left(e_{23}^{T}-e_{11}^{T}\right)
\right].
\]
Der chemische Term beschreibt hierbei den eigentlichen Trennnutzen infolge der erhöhten Reinheit, während der thermische Term den zusätzlichen Nutzen des kryogenen Zustands erfasst. 
In der zweiten Variante wird neben dem Destillat auch der Bottomstrom $S12$ als Ko-Produkt berücksichtigt. Dieser Strom entsteht zwangsläufig aus dem Trennprozess und trägt ebenfalls chemische sowie thermische Exergie aus dem System. Obwohl er im vorliegenden Anwendungsszenario nicht unmittelbar genutzt wird, stellt seine Exergie keinen irreversiblen Verlust dar. Die exergetische Produktleistung setzt sich daher aus Hauptprodukt und Ko-Produkt zusammen:
\[
\dot{E}_P^{(2)}
=
\dot{m}_{23}
...
\dot{m}_{12}
\left[
\left(e_{12}^{CH}-e_{11}^{CH}\right)
+
\left(e_{12}^{T}-e_{11}^{T}\right)
\right].
\]
Die gesamte abgeführte Produkt-Exergie ergibt sich folglich zu
\[
\dot{E}_{P,\mathrm{ges}}^{(2)} = \dot{E}_P^{(2)} + \dot{E}_{co}^{(2)}.
\]
Die erste Variante bewertet somit ausschließlich den funktionalen Zielnutzen der Stickstoffbereitstellung, während die zweite Variante die gesamte thermodynamische Trennleistung der Kolonne unabhängig von der späteren Nutzung einzelner Ströme beschreibt. Beide Definitionen sind thermodynamisch konsistent; ihre Anwendung hängt von der jeweiligen Systemgrenze und dem Bewertungsziel ab. Während Variante~(1) die prozessspezifische Zielerfüllung charakterisiert, erlaubt Variante~(2) eine faire Beurteilung der intrinsischen Effizienz des Trennapparates ohne wirtschaftliche oder funktionale Vorannahmen.
...
\begin{table}[H]
\centering
\caption{Verwendete Zuschläge für PEC-basierte Kostenblöcke in Anlehnung an Tesch und Bejan et al.}
